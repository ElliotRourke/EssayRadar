 ...
 * This class is responsible for the parsing of '.docx' files uploaded by the user.
 * It uses the Apache POI library to parse the files easily and remove frivolous data or data that is harmful to the results.
 * @see Parser
 */
public class DocParser extends Parser {
	...
	public void parse(String path) throws FileNotFoundException, IOException {
		...
		String[] tokenTerms = we.getText().toString().toLowerCase().replaceAll("[\\W&&[^\\s]]","").split("\\W+");
